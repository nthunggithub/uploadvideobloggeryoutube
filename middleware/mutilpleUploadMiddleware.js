...
let storage = multer.diskStorage({
  ...
  filename: (req, file, callback) => {
    ...
    let math = ["video/mp4", "video/x-flv", "video/x-ms-wmv", "video/x-msvideo", "video/quicktime"];
    if (math.indexOf(file.mimetype) === -1) {
      let errorMess = `The file <strong>${file.originalname}</strong> is invalid. Only allowed to upload video.`;
...
let uploadManyFiles = multer({storage: storage}).array("many-files", 50);
